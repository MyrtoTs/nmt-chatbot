...
#         source = tf.Print(iterator.source, [iterator.source], summarize=100)
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            with open('/home/myrto/pink_dir/Chatbot308/myrto-tsokanaridou-chatbot/topic_modeling/ids_to_LDA_vectors.pickle', 'rb') as handle:
                mapper = pickle.load(handle)

            vf = np.vectorize(lambda id, pos: np.float32(mapper[id][pos]))

            def concat_coord(x):
                np.concatenate((vf(x, 0), vf(x, 1), vf(x, 2), vf(x, 3), vf(x, 4), vf(x, 5), vf(x, 6), vf(x, 7), vf(x, 8), vf(x, 9), 
                                vf(x, 10), vf(x, 11), vf(x, 12), vf(x, 13), vf(x, 14), vf(x, 15), vf(x, 16), vf(x, 17), vf(x, 18), 
                                vf(x, 19)),axis=2)           
            ...
            encoder_emb_inp.set_shape((None, None, 532))
