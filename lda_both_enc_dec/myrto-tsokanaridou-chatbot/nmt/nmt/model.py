    ...
    def _build_decoder(self, encoder_outputs, encoder_state, hparams):
        ...
        with tf.variable_scope("decoder") as decoder_scope:
            ...
            if self.mode != tf.contrib.learn.ModeKeys.INFER:
                ...
                with open('/home/myrto/pink_dir/github/lda_chatbot/myrto-tsokanaridou-chatbot/topic_modeling/ids_to_LDA_vectors.pickle', 'rb') as handle:
                    mapper = pickle.load(handle)

                vf = np.vectorize(lambda id, pos: np.float32(mapper[id, pos]))

                def concat_coord(x):
                    return np.concatenate((vf(x, 0), vf(x, 1), vf(x, 2), vf(x, 3), vf(x, 4), vf(x, 5), vf(x, 6), vf(x, 7), vf(x, 8), vf(x, 9),vf(x, 10), vf(x, 11), vf(x, 12), vf(x, 13), vf(x, 14), vf(x, 15), vf(x, 16), vf(x, 17), vf(x, 18),vf(x, 19), vf(x, 20), vf(x, 21), vf(x, 22), vf(x, 23), vf(x, 24), vf(x, 25), vf(x, 26), vf(x, 27),vf(x, 28), vf(x, 29), vf(x, 30), vf(x, 31), vf(x, 32), vf(x, 33), vf(x, 34), vf(x, 35), vf(x, 36), vf(x, 37), vf(x, 38), vf(x, 39)),axis=2)   
                ...
                decoder_emb_inp.set_shape((None, None, 552))
...
class Model(BaseModel):
    ...
    def _build_encoder(self, hparams):
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            encoder_emb_inp.set_shape((None, None, 552))
